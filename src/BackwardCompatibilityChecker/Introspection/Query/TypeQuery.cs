...
namespace BackwardCompatibilityChecker.Introspection.Query
{
    ...
    public class TypeQuery
    {
        ...
        public TypeQuery(TypeQueryMode mode, string namespaceFilter, string typeNameFilter)
        {
            ...
            if (!this.IsEnabled(TypeQueryMode.Class) && !this.IsEnabled(TypeQueryMode.Interface) && !this.IsEnabled(TypeQueryMode.ValueType) && 
                !this.IsEnabled(TypeQueryMode.Enum))
            {
                this.myQueryMode |= TypeQueryMode.Class | TypeQueryMode.Interface | TypeQueryMode.ValueType |
                               TypeQueryMode.Enum;
            }
            ...
            if (this.CheckVisbility(typeDef))
            {
                ...
                if (this.IsEnabled(TypeQueryMode.NotCompilerGenerated))
                {
                    if( typeDef.IsSpecialName ||
                        typeDef.Name == "<Module>" ||
                        this.HasCompilerGeneratedAttribute(typeDef)
                      )
                    {
                        goto End;
                    }
