...
{
    /// <summary>
    /// Parser for a list of type queries separated by ;.
    /// </summary>
    internal sealed class TypeQueryFactory
    {
        static readonly Regex MyQueryParser = new Regex("^ *(?<modifiers>api +|nocompiler +|public +|internal +|class +|struct +|interface +|enum +)* *(?<typeName>[^ ]+) *$");

        public List<TypeQuery> GetQueries(string typeQueries, TypeQueryMode additionalFlags = TypeQueryMode.None)
        {
            if (typeQueries == null)
            {
        ...
        private KeyValuePair<string, string> SplitNameSpaceAndType(string fullQualifiedTypeName)
        {
            ...
            string[] parts = fullQualifiedTypeName.Trim().Split(new[] { '.' });
            if (parts.Length > 1)
            {
        ...
        private bool Captures(Match m, string value)
        {
