...
namespace BackwardCompatibilityChecker.Infrastructure.Diagnostics
{
    ...
    {
        static readonly string Help = "Tracing can be enabled via the environment variable " + TracerConfig.TraceEnvVarName + Environment.NewLine +
                                      "Format: <Output Device>;<Type Filter> <Message Filter>; <Type Filter> <Message Filter>; ..." + Environment.NewLine +
                                      "    <Output Device>  Can be: Default, File, Debugoutput or Console." + Environment.NewLine +
                                      "                       Default             Write traces to configured trace listeners with the source name " + TracerConfig.TraceEnvVarName + " read from App.config" + Environment.NewLine + 
                                      "                       File [FileName]     Write traces to given file name. The AppDomain name and process id are prepended to the file name to make it unique. If none given a trace file where the executable is located is created" + Environment.NewLine +
                                      "                       DebugOutput         Write to windows kernel via OutputDebugString which can be best viewed with dbgview from SysInternals" + Environment.NewLine  +
                                      "                       Console             Write to stdout" + Environment.NewLine + 
                                      "    <[!]TypeFilter>  It is basically the full qualified type name (case insensitive)." + Environment.NewLine +
                                      "                     If the TypeFilter begins with a ! character it is treated as exclusion filter." + Environment.NewLine +
                                      "                     Example: ApiChange.Infrastructure.AggregateException or * or ApiChange.* partial name matches like Api* are NOT supported" + Environment.NewLine +
                                      "                     " + Environment.NewLine +
